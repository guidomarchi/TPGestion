...
IF OBJECT_ID('[4_FILAS_AFECTADAS].CARGAR_PUBLICACION') IS NOT NULL DROP PROCEDURE [4_FILAS_AFECTADAS].CARGAR_PUBLICACION
IF OBJECT_ID('[4_FILAS_AFECTADAS].CARGAR_DETALLE_FACTURA') IS NOT NULL DROP PROCEDURE [4_FILAS_AFECTADAS].CARGAR_DETALLE_FACTURA
IF OBJECT_ID('[4_FILAS_AFECTADAS].CARGAR_DETALLE_VENTA') IS NOT NULL DROP PROCEDURE [4_FILAS_AFECTADAS].CARGAR_DETALLE_VENTA
IF OBJECT_ID('[4_FILAS_AFECTADAS].CARGAR_PAGO') IS NOT NULL DROP PROCEDURE [4_FILAS_AFECTADAS].CARGAR_PAGO
IF OBJECT_ID('[4_FILAS_AFECTADAS].CARGAR_DETALLE_PAGO') IS NOT NULL DROP PROCEDURE [4_FILAS_AFECTADAS].CARGAR_DETALLE_PAGO
IF OBJECT_ID('[4_FILAS_AFECTADAS].CARGAR_USUARIOXDOMICILIO') IS NOT NULL DROP PROCEDURE [4_FILAS_AFECTADAS].CARGAR_USUARIOXDOMICILIO
...
create table [4_FILAS_AFECTADAS].UsuarioXDomicilio(
	usu_id int,
	dom_id int,
	primary key (usu_id, dom_id),
);
GO
...
CREATE PROCEDURE [4_FILAS_AFECTADAS].CARGAR_USUARIOXDOMICILIO
AS 
BEGIN
PRINT 'Tabla UsuarioXDomicilio'
insert into [4_FILAS_AFECTADAS].UsuarioXDomicilio(usu_id, dom_id)
select distinct usu_id, dom_id from GD2C2024.gd_esquema.Maestra m 
	join [4_FILAS_AFECTADAS].Usuario u on m.VEN_USUARIO_NOMBRE = u.usu_nombre and m.VEN_USUARIO_PASS = u.usu_contrasenia and m.VEN_USUARIO_FECHA_CREACION = u.usu_fecha_creacion
	join [4_FILAS_AFECTADAS].Domicilio d on m.VEN_USUARIO_DOMICILIO_CALLE = d.dom_calle and m.VEN_USUARIO_DOMICILIO_NRO_CALLE = d.dom_no_calle and m.VEN_USUARIO_DOMICILIO_PISO = d.dom_piso and m.VEN_USUARIO_DOMICILIO_DEPTO = d.dom_depto and m.VEN_USUARIO_DOMICILIO_CP = d.dom_cp
	join [4_FILAS_AFECTADAS].Localidad l on l.loc_id = d.dom_loc
	join [4_FILAS_AFECTADAS].Provincia p on p.prov_id = l.loc_prov
union
select distinct usu_id, dom_id from GD2C2024.gd_esquema.Maestra m 
	join [4_FILAS_AFECTADAS].Usuario u on m.CLI_USUARIO_NOMBRE = u.usu_nombre and m.CLI_USUARIO_PASS = u.usu_contrasenia and m.CLI_USUARIO_FECHA_CREACION = u.usu_fecha_creacion
	...
	join [4_FILAS_AFECTADAS].Localidad l on l.loc_id = d.dom_loc
	join [4_FILAS_AFECTADAS].Provincia p on p.prov_id = l.loc_prov
where usu_id is not null and dom_id is not null
END
GO
...
-- Agregar FK en Publicacion que referencia a AlmacenXProducto
ALTER TABLE [4_FILAS_AFECTADAS].Publicacion
...
-- Agregar FK en UsuarioXDomicilio que referencia a Usuario
ALTER TABLE [4_FILAS_AFECTADAS].UsuarioXDomicilio 
ADD CONSTRAINT FK_UsuarioXDomicilio_Usuario FOREIGN KEY (usu_id) 
	REFERENCES [4_FILAS_AFECTADAS].Usuario (usu_id);
GO

-- Agregar FK en UsuarioXDomicilio que referencia a Domicilio
ALTER TABLE [4_FILAS_AFECTADAS].UsuarioXDomicilio 
